...
1.  **Compilar o projeto Spring Boot**: Executar o comando `mvn clean package` para criar o arquivo JAR na pasta `target`.
    
2.  **Criar a Imagem Docker**: No diretório do projeto, com o Dockerfile, executar:
...
3.  **Testar a Imagem Docker**: Para garantir que a imagem foi criada corretamente, rodar localmente:
    
    ```bash
    docker run -p 8080:8080 usuario/projeto
    
    ```
    
4.  **Configurar no Render**:
    
    -   Criar um Dockerfile para produção, na raiz do projeto e comitar para o repositório GitHub. Exemplo de Dockerfile:
    ...
    -   Criar um novo WebService.
    -   Escolha a opção "Docker" e conectar o repositório GitHub.
    -   O Render deve detectar automaticamente o Dockerfile e construir a imagem.
5.  **Definir as Variáveis de Ambiente** (se necessário):
    
    -   No painel do Render, nas configurações do serviço, definir as variáveis de ambiente necessárias para aplicação Spring Boot (como `DATABASE_URL`, `SPRING_PROFILES_ACTIVE`, etc).

[Este vídeo](https://render.com/) me ajudou a compreender o processo de *deploy* no Render, embora não tenha seguido à risca todos os passos por não se aplicar completamente a este projeto.
